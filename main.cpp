...
struct ObjectLayers
{
    let NON_MOVING = JPH::ObjectLayer(0);
    let MOVING = JPH::ObjectLayer(1);
    let NUM = 2;
};

struct BroadPhaseLayers
{
    let NON_MOVING = JPH::BroadPhaseLayer(0);
    let MOVING = JPH::BroadPhaseLayer(1);
    let NUM = 2;
};
...
fn jolt_trace(const char* fmt, ...) -> void
{
    va_list list;
    va_start(list, fmt);
    char buffer[1024];
...
fn jolt_assert_failed(const char* expression, const char* message, const char* file, u32 line) -> bool
{
...
struct GameState
{
    let WINDOW_NAME = "game";
    let WINDOW_WIDTH = 1200;
    let WINDOW_HEIGHT = 800;
    let NUM_GPU_PIPELINES = 1;
    let GPU_BUFFER_SIZE_STATIC = 8 * 1024 * 1024;
    let GPU_BUFFER_SIZE_DYNAMIC = 256 * 1024;
    ...
    fn init(GameState* game_state) -> void {
        assert(game_state);
        ...
        if (!GpuContext::init(game_state->gpu.gc, window)) {
    ...
    fn deinit(GameState* game_state) -> void {
        assert(game_state);

        if (game_state->gpu.gc) GpuContext::finish(game_state->gpu.gc);
        ...
        if (game_state->gpu.gc) {
            GpuContext::deinit(game_state->gpu.gc);
            delete game_state->gpu.gc;
            game_state->gpu.gc = nullptr;
        }
    }

    fn draw_frame(GameState* game_state) -> void {
        assert(game_state);
        ...
        GpuContext::present(gc);
    }

    fn update(GameState* game_state) -> void {
        assert(game_state);

        game_state->is_window_minimized = !GpuContext::handle_window_resize(game_state->gpu.gc);
        if (game_state->is_window_minimized)
            return;
    ...
    fn draw(GameState* game_state) -> void {
        assert(game_state);
...
i32 main()
{
    ...
    defer { GameState::deinit(game_state); };
    ...
    while (true) {
        ...
        if (PeekMessage(&msg, nullptr, 0, 0, PM_REMOVE)) {
            ...
            GameState::update(game_state);
            GameState::draw_frame(game_state);
        }
    }
