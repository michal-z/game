...
func create_msaa_srgb_render_target(GpuContext* gc) -> void
{
    assert(gc && gc->device);
    ...
    gc->device->CreateRenderTargetView(gc->msaa_srgb_rt, nullptr, { .ptr = gc->rtv_heap_start.ptr + GPU_MAX_BUFFERED_FRAMES * gc->rtv_heap_descriptor_size });
...
func init_gpu_context(GpuContext* gc, HWND window) -> bool
{
    ...
    const D3D12_DESCRIPTOR_HEAP_DESC gpu_heap_desc = {
        ...
        .NumDescriptors = GPU_MAX_DESCRIPTORS,
...
func shutdown_gpu_context(GpuContext* gc) -> void
{
    ...
    for (i32 i = 0; i < GPU_MAX_BUFFERED_FRAMES; ++i) SAFE_RELEASE(gc->command_allocators[i]);
    ...
    for (i32 i = 0; i < GPU_MAX_BUFFERED_FRAMES; ++i) SAFE_RELEASE(gc->swap_chain_buffers[i]);
...
func finish_gpu_commands(GpuContext* gc) -> void
{
    assert(gc && gc->device);
...
func handle_window_resize(GpuContext* gc) -> bool
{
    assert(gc && gc->device);
...
func present_frame(GpuContext* gc) -> void
{
    assert(gc && gc->device);
    gc->frame_fence_counter += 1;
